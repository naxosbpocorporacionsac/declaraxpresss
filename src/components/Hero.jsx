export default function Hero() {
  return (
    <header className="hero" id="top">
      <div className="hero-bg">
        <img src="/assets/hero.jpg" alt="" className="hero-bg-img" />
        <div className="hero-overlay"></div>
      </div>
      <div className="hero-inner">
        <img className="hero-logo" src="/assets/declara-xpress-logo.png" alt="Declara Xpress" />
        <h1>En DeclaraXpress.com tenemos planes de servicios contables para tu negocio desde <em>S/170 al mes</em>, incluido IGV.</h1>
        <p className="hero-subtitle">Tu contabilidad organizada sin errores y tus impuestos bajo control con <strong>IA</strong>. Con planes adecuados para tu empresa.</p>
        <a className="btn btn-coral hero-cta-btn" href="#planes">Elegir mi plan</a>
      </div>
    </header>
  );
}
